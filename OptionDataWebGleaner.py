...
def plot(data, N=20):
    ...
    for t in data:
        ...
        fig.suptitle('Option Data Summary for ' + t, fontsize=20)
        plot1 = plt.subplot(2, 2, 1)
        plot1.bar(np.arange(len(data[t])), data[t]['IV'], barwidth, facecolor='#9999ff', edgecolor='white',
                  label=t)
        plot1.set_title('Implied Volatility Distribution', fontsize=12)
        plt.ylabel('Implied Volatility'
                   '')
        plot2 = plt.subplot(2, 2, 2)
        plot2.bar(np.arange(len(data[t])), data[t]['Last'], barwidth, facecolor='#9999ff', edgecolor='white',
                  label=t)
        plot2.set_title('Option Price Distribution', fontsize=12)
        plt.ylabel('Option Price')

        plot3 = plt.subplot(2, 1, 3)
        plot3.plot(data[t]['Last'], data[t]['IV'], label=t)
        plot3.legend(loc='upper left')
        plot3.set_title("Option Price vs Implied Volatility", fontsize=12)
        plt.xlabel('Option Price')
        plt.ylabel('Implied Volatility')
        fig.savefig('Data Visualization for ' + str(t) + '.png')
        if count < N:
            count += 1
        else:
            break
